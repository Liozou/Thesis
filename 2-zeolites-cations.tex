...
\begin{figure}
	...
	\caption{Cation location for FAU with $\SiAl = 1$}\label{fig:FAU}
\end{figure}
...
To keep it more physical, the Voronoi cells are intersected with solid spheres centered on the site and of radius \qty{2}{\angstrom}. Thus, any point in space is either in one of the truncated cells, in which case it is associated with a site closer than \qty{2}{\angstrom}, or it is not associated with any site. Once established for a given topology, this map is used for all its derivatives.

In the literature, the different sites of FAU are well-known and have been given names. These are represented in \cref{fig:FAUsites}, using one color per kind of site, and with size proportional to their occupancy.

%TODO pathological and ideal cases
...
Since both values of $S_{\boldsymbol k}^\text{guest}2$ need to be computed before and after the move to take the difference of their norm, it is actually equally efficient to directly compute both values of $E^\text{reciprocal}$ before and after the move, directly following \cref{eq:reciprocal_decomposed}, and taking the difference, instead of the previous formulation.
