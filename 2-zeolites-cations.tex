...
To avoid drift, the algorithm obeys detailed balance which, conceptually, represents the microreversibility of the simulated physical processes. In practise, this means that, for each MC move that brings the system from configuration ${\bf p}_A$ to ${\bf p}_B$, there must exist a ``converse'' MC moves that can bring the system from $B$ to $A$ such that $\pi_A P_{A\to B} = \pi_B P_{B\to A}$ where $\pi_X$ is the stationary probability of state ${\bf p}_X$ and $P_{X\to Y}$ is the probability of the move from ${\bf p}_X$ to ${\bf p}_Y$. In the canonical ensemble, $\pi_X$ is equal to to $\frac1Z\exp\left(-\beta U\left({\bf p}_X\right)\right)$, while the probability of transition is $\min\left(1, \exp\left(-\beta \Delta E\right)\right)$. Hence, detailed balance simply translates to the algorithmic condition that the probability of choosing the trial move $A\to B$ must be equal to that of choosing the trial move $B\to A$, irrespective of the probability of accepting these moves.
